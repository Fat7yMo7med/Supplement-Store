...
function CoachesProfiles() {
    ...
    return (
        ...
        <div className="container">
            <button className="btn btn-outline-cyan mb-4" onClick={() => navigate('/coaches')}> ← Back to Coaches </button>

            <div className="row g-4">
            <div className="col-lg-4">
                <div className="card-section text-center shadow">
                    <img src={coach.image} alt={coach.name} className="img-fluid rounded mb-3" style={{ height: "300px", width: "100%", objectFit: "cover" }}/>
                    <h3 className="text-cyan fw-bold">{coach.name}</h3>
                    <p className="text-secondary">{coach.title}</p>
                    <span className="badge badge-cyan">{coach.specialty}</span>
                </div>
            </div>
                    
                <div className="col-lg-8">
                    <div className="card-section shadow">
                        <h4 className="text-cyan">About {coach.name.split(" ")[0]}</h4>
                        <p className="mt-3">{coach.fullBio}</p>
                    </div>
                        <div className="card-section shadow">
                            <h4 className="text-cyan">Professional Details</h4>
                        <div className="row text-center mt-3">
                            <div className="col-4">
                                <h3 className="text-cyan">{coach.experience.split(" ")[0]}</h3>
                                <p>Years of Experience</p>
                            </div>
                            <div className="col-4">
                                <h3 className="text-cyan">{coach.clients.split(" ")[0]}</h3>
                                <p>Clients</p>
                            </div>
                            <div className="col-4">
                                <h3 className="text-cyan">{coach.certifications.split(" ")[0]}</h3>
                                <p>Certifications</p>
                            </div>
                        </div>
                    </div>
                        <div className="card-section shadow mt-4">
                            <h4 className="text-cyan">Contact Information</h4>
                                <div className="row text-center mt-3">
                                <div className="col-6">
                                    <h5 className="text-cyan">Phone</h5>
                                    <p className="mt-2">{coach.phone}</p>
                                </div>
                                <div className="col-6">
                                    <h5 className="text-cyan">Email</h5>
                                    <p className="mt-2">{coach.email}</p>
                                </div>
                            </div>
                        </div>
                    </div>
