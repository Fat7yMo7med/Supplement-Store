  ...
  return (
    <div className="container-fluid p-4" style={{ background: "#0f172a", color: "#e0e0e0", minHeight: "100vh" }}>
      ...
      <style>
        ...
        .form-control-cyan:focus { border-color: #00e5ff; box-shadow: 0 0 8px #00e5ff; backimport { Toaster } from 'react-hot-toast';
ground-color: #1e293b; color: #ffffff; }
        .thumb.active { border: 2px solid #00e5ff; }
        ...
        <div className="col-12 col-md-6">
          <div className="card p-3 card-bg">
            ...
            <div className="mb-3">
              ...
              {selected.oldPrice && <small className="text-warning">(was ${selected.oldPrice})</small>}
            </div>
            ...
            <ul className="nav nav-tabs">
              <li className="nav-item">
                <a href="#" className={`nav-link text-warning ${tab === "description" ? "active" : ""}`} onClick={e => { e.preventDefault(); setTab("description"); }}>Description</a>
              </li>
              <li className="nav-item">
                <a href="#" className={`nav-link text-warning ${tab === "additional" ? "active" : ""}`} onClick={e => { e.preventDefault(); setTab("additional"); }}>Additional Info</a>
              </li>
              <li className="nav-item">
                <a href="#" className={`nav-link text-warning ${tab === "reviews" ? "active" : ""}`} onClick={e => { e.preventDefault(); setTab("reviews"); }}>Reviews ({selected.reviews?.length || 0})</a>
              </li>
            </ul>
                  ...
                  {selected.reviews?.length ? selected.reviews.map((rev, idx) => (
                    <div key={idx} className="mb-3">
                      ...
                      <div className="text-warning">
                        {Array.from({ length: rev.rating }, (_, i) => <i key={i} className="fas fa-star star"></i>)}
                        {rev.rating < 5 && Array.from({ length: 5 - rev.rating }, (_, i) => <i key={i} className="fas fa-star-half-alt star"></i>)}
                      </div>
      ...
      <div className="row row-cols-2 row-cols-sm-3 row-cols-md-4 g-3">
        {related.map(p => ( <div className="col" key={p.id}>
