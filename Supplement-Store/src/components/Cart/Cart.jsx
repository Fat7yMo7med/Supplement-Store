...
export default function CartPage() {
  ...
  const handleRemove = (id, name) => {
    removeFromCart(id);
    toast.error(`${name} removed from cart`, { style: toastStyle });
  };

  const handleDecrease = (id, name) => {
    decreaseQuantity(id);
    toast(`${name} quantity decreased`, { style: toastStyle, icon: "➖" });
  };

  const handleIncrease = (item) => {
    addToCart({ ...item, quantity: 1 });
    toast.success(`${item.name} quantity increased`, { style: toastStyle });
  };

  const handleClear = () => {
    clearCart();
    toast.error(`Cart cleared!`, { style: toastStyle });
  };
            ...
            <div className="col-md-8">
              ...
              {cart.length === 0 ? (
                <div className="card cart-card mb-4 shadow">
                      ...
                      {cart.map(item => (
                        <div key={item.id} className="row mb-4 align-items-center">
                          ...
                          <div className="col-lg-5">
                            ...
                            <button className="btn bg-danger btn-sm me-2" onClick={() => handleRemove(item.id, item.name)} >
                              <i className="fas fa-trash"></i>
                            </button>
                          </div>
                          <div className="col-lg-4">
                            <div className="d-flex mb-3" style={{ maxWidth: "200px" }}>
                              <button className="btn btn-cyan px-3 me-2" onClick={() => handleDecrease(item.id, item.name)}>
                                <i className="fas fa-minus"></i>
                              </button>
                              <input type="number" min="1" value={item.quantity} className="form-control text-center" readOnly/>
                              <button className="btn btn-cyan px-3 ms-2" onClick={() => handleIncrease(item)}>
                  ...
                  <div className="card cart-card shadow mb-3">
                    <div className="card-body">
                      ...
                      <div className="d-flex gap-2 flex-column flex-md-row mt-3">
                        <button className="btn bg-danger btn-lg w-100" onClick={handleClear}>
                          Clear Cart
                        </button>
                        <button className="btn btn-cyan btn-lg w-100" onClick={handleCheckout}>
                          Proceed to Checkout
                        </button>
                      </div>
                    </div>
